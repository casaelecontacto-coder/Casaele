...
export default function Embeds() {
  ...
  function Preview({ code }) {
    ...
    if (!code) return null
    return (
      <div className="relative rounded-xl border border-casa-ink/12 bg-white overflow-hidden shadow-sm">
        {isLoading && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/60 backdrop-blur-sm">
            <svg className="h-6 w-6 animate-spin text-casa-red" viewBox="0 0 24 24">
  ...
  return (
    <div className="flex gap-6 min-h-screen">
      ...
      <div className="w-64 flex-shrink-0 space-y-4">
        <div className="rounded-xl bg-white shadow-sm border border-casa-ink/12 p-4">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-casa-ink">Chapters</h2>
            <button
              onClick={() => { setShowChapterModal(true); setEditingChapterId(null); setChapterForm({ name: '', description: '' }); }}
              className="text-sm text-casa-red hover:underline"
            >
              + New
            </button>
          </div>
            ...
            <button
              ...
              className={`w-full text-left px-3 py-2 rounded-xl text-sm transition ${
                selectedChapter === null
                  ? 'bg-casa-red/8 text-casa-red font-medium'
                  : 'text-casa-ink/75 hover:bg-casa-cream/40'
              }`}
            >
              All Embeds
            </button>
              ...
              <div key={chapter._id} className="group">
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="ml-2 text-xs text-casa-ink/50">({chapter.embedCount || 0})</span>
                  </div>
                </button>
                <div className="hidden group-hover:flex items-center gap-1 mt-1 px-3">
                  <button onClick={() => onEditChapter(chapter)} className="text-xs text-casa-ink/65 hover:text-casa-ink">Edit</button>
                  <span className="text-gray-300">•</span>
                  <button onClick={() => onExportChapter(chapter._id)} className="text-xs text-casa-ink/65 hover:text-casa-ink">Export</button>
                  <span className="text-gray-300">•</span>
                  <button onClick={() => onDeleteChapter(chapter._id)} className="text-xs text-casa-red hover:text-red-900">Delete</button>
                </div>
              </div>
            ))}
          </div>
          ...
          {chapters.length > 0 && (
            <button
              ...
              className="mt-4 w-full px-3 py-2 text-sm text-casa-ink/75 bg-casa-cream/40 rounded-xl hover:bg-casa-cream/60 transition"
            >
              📊 Export All Chapters
            </button>
          )}
      ...
      <div className="flex-1 space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-semibold text-casa-ink">
            {selectedChapter === null
              ? 'All Embeds'
              : selectedChapter === 'uncategorized'
        ...
        {error && <div className="text-sm text-casa-red bg-casa-red/8 border border-casa-red/25 rounded p-3">{String(error)}</div>}

        {/* Form */}
        <form onSubmit={onSubmit} className="rounded-xl bg-white shadow-sm border border-casa-ink/12 p-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full rounded-xl border border-casa-ink/20 px-4 py-2 focus:border-casa-red focus:ring-2 focus:ring-red-200 transition-all shadow-sm outline-none"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-casa-ink/75 mb-1">Type</label>
              <select
                name="type"
                value={form.type}
                onChange={onChange}
            ...
            <div>
              <label className="block text-sm font-medium text-casa-ink/75 mb-1">Chapter</label>
              <select
                name="chapterId"
            ...
            <div className="md:col-span-3">
              <label className="block text-sm font-medium text-casa-ink/75 mb-1">Upload HTML Activity File</label>
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="flex-1 inline-flex items-center justify-center px-4 py-2 rounded-xl border-2 border-dashed border-casa-ink/20 bg-casa-cream/40 text-casa-ink/75 hover:bg-casa-cream/60 hover:border-gray-400 transition"
                >
                ...
                <button
                  ...
                  className="inline-flex items-center px-4 py-2 rounded-xl bg-casa-red text-white hover:bg-casa-redDark transition disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {uploadingHtml ? (
                    <>
              ...
              <p className="mt-1 text-xs text-casa-ink/50">Upload an HTML file to automatically add its URL to the embed code field</p>
            </div>

            <div className="md:col-span-3">
              <label className="block text-sm font-medium text-casa-ink/75 mb-1">Embed Code / URL</label>
              <textarea
                name="embedCode"
                rows="5"
                ...
                className="w-full rounded-xl border border-casa-ink/20 px-4 py-2 focus:border-casa-red focus:ring-2 focus:ring-red-200 transition-all shadow-sm outline-none resize-none"
              />
              <p className="mt-1 text-xs text-casa-ink/50">You can paste an iframe, script tag, or a direct URL to an HTML file</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button
              type="submit"
              className="inline-flex items-center px-4 py-2 rounded-xl bg-casa-red text-white hover:bg-casa-redDark transition"
            >
            ...
            {editingId && (
              <button
                ...
                className="text-sm text-casa-ink/65 hover:text-casa-ink"
              >
                Cancel edit
              </button>
            )}
        ...
        {/* List */}
        <div className="rounded-xl bg-white shadow-sm border border-casa-ink/12">
          {/* Search and Sort Controls */}
          <div className="px-4 py-3 border-b border-casa-ink/12 space-y-3">
            <div className="flex items-center justify-between">
              <div className="text-sm font-medium text-casa-ink/75">
                Embeds ({filteredAndSortedItems.length}{searchTerm && ` of ${items.length}`})
              </div>
              <button onClick={fetchEmbeds} className="text-sm text-casa-red hover:underline">Refresh</button>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 rounded-xl border border-casa-ink/20 text-sm focus:border-casa-red focus:ring-2 focus:ring-red-200 transition-all outline-none"
                />
                {searchTerm && (
                  <button
                    onClick={() => setSearchTerm('')}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-casa-ink/40 hover:text-gray-600"
                  >
              ...
              <select
                ...
                className="px-4 py-2 rounded-xl border border-casa-ink/20 text-sm focus:border-casa-red focus:ring-2 focus:ring-red-200 transition-all outline-none bg-white min-w-[160px]"
              >
          ...
          <div className="divide-y divide-gray-100">
            {loading && <div className="p-4 text-sm text-casa-ink/50">Loading…</div>}
            {!loading && filteredAndSortedItems.length === 0 && (
              <div className="p-4 text-sm text-casa-ink/50">
                ...
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2">
                      <div className="text-sm font-medium text-casa-ink">{item.title}</div>
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => onEdit(item)} className="text-sm text-casa-ink/75 hover:underline">Edit</button>
                    <button
                      onClick={() => handleToggleActive(item._id, item.isActive)}
                      className={item.isActive === false ? "text-yellow-600 hover:text-yellow-800" : "text-casa-ink/40 hover:text-gray-600"}
                    ...
                    <button onClick={() => onDelete(item._id)} className="text-sm text-casa-red hover:underline">Delete</button>
                  </div>
                </div>
          ...
          {filteredAndSortedItems.length > pageSize && (
            <div className="px-4 py-3 border-t border-casa-ink/12 flex items-center justify-between text-sm">
              <div className="text-casa-ink/50">Page {page} of {Math.ceil(filteredAndSortedItems.length / pageSize)}</div>
              <div className="flex items-center gap-2">
                <button
                  disabled={page === 1}
                  onClick={() => setPage(p => Math.max(1, p - 1))}
                  className="px-3 py-1 rounded border text-casa-ink/75 disabled:opacity-50"
                >
                  Prev
                </button>
                <button
      ...
      {showChapterModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md">
            <h2 className="text-xl font-semibold text-casa-ink mb-4">
              {editingChapterId ? 'Edit Chapter' : 'New Chapter'}
            </h2>
            <form onSubmit={onSubmitChapter} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-casa-ink/75 mb-1">Chapter Name</label>
                <input
                  name="name"
              ...
              <div>
                <label className="block text-sm font-medium text-casa-ink/75 mb-1">Description (Optional)</label>
                <textarea
                  name="description"
